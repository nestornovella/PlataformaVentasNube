...
import { error, response, responseError, statusCode } from "../helpers/ResponseHelper.helper";
...
export async function updateUser(req:NextRequest,{params}:paramsInterface){
    try {
        ...
        const updated = await models.User.update(body, {where:{id:params.id}})
...
export async function createUser(req:NextRequest){
    ...
    try {
        ...
        if(!email || ! name || !password) error('parametros requeridos no proporcionados')
        await checkConection()
